...
from .permission import HostListPermission, My_Permission
from .serializers import (EventParticipantSerializer, EventSerializer,
                          FeedbackSerializer, InvitationSerializer,
                          RegisterSerializer)
...
class EventListCreateView(ListCreateAPIView):
    ...
    def post(self, request):
        try:
            ...
            events_today = Event.objects.filter(
                host=request.user, created_at__date=today
            ).count()

            if events_today >= 5:
                return Response(
                    {"detail": "You can only create 5 events per day."},
    ...
    def get_queryset(self):
        ...
        try:
            ...
            return Response(
                {"error": f"Could not fetch events: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class EventRetrieveUpdateDestroyView(RetrieveUpdateDestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"error": f"There is No Event Exists for that Host: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class EventParticipantCreate(APIView):
    ...
    def post(self, request, event_id):
        ...
        if event.max_participants <= event.eventparticipant_set.count():
            return Response(
                {"error": "Event is full"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ListInvitationsView(APIView):
    ...
    def get(self, request):
        ...
        invitations = Invitation.objects.filter(
            event__in=hosted_events, status="PENDING"
        )
...
class RespondInvitationView(UpdateAPIView):
    ...
    def get_object(self):
        invitation = Invitation.objects.filter(
            event_id=self.kwargs["event_id"], invitee=self.request.user
        ).first()
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            self.get_serializer(invitation).data, status=status.HTTP_200_OK
        )
...
class GetUserDestroyListView(RetrieveDestroyAPIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            registered_events = Event.objects.filter(
                eventparticipant__user=request.user
            )
    ...
    def delete(self, request, event_id):
        try:
            ...
            participant.delete()
            return Response(
                {"message": "You are unregistered from the event"},
                status=status.HTTP_204_NO_CONTENT,
            )

        except Exception as e:
            return Response(
                {"error": f"something wrong :--->{str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class DeleteOldEvents(APIView):

    def post(self, request):
        try:
            call_command("eventdelete")
            return Response(
                {"message": "Success check terminal "},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response(
